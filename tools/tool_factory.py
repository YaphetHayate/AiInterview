...
def create_session_tools(session_config: dict) -> list:
    @tool
    def read_session_config() -> str:
        """获取当前面试会话的配置信息，包括技术栈、难度、风格、岗位、模式等。"""
        lines = ["当前面试会话配置："]
        if "tech_stack" in session_config:
            lines.append(f"- 技术栈：{session_config['tech_stack']}")
        if "difficulty" in session_config:
            lines.append(f"- 难度：{session_config['difficulty']}")
        if "interview_style" in session_config:
            lines.append(f"- 面试风格：{session_config['interview_style']}")
        if "position" in session_config:
            lines.append(f"- 岗位：{session_config['position']}")
        if "mode" in session_config:
            lines.append(f"- 面试模式：{session_config['mode']}")
        return "\n".join(lines)
    ...
    return [read_session_config, read_skill_md, read_stage_file, fetch_questions_from_bank]
